...
export default function EditEHRPage({ 
  ...
  const [formData, setFormData] = useState<EHRRecord>({
    _id: params.ehrId,
    patientId: params.id,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch EHR record data
      const ehrResponse = await fetch(`/api/ehr/${params.ehrId}`);
      if (!ehrResponse.ok) throw new Error("Failed to fetch EHR record");
      const ehrData = await ehrResponse.json();
      
      // Format the visit date for the input field
      const visitDate = new Date(ehrData.visitDate)
        .toISOString()
        .split("T")[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to update EHR record");
      }
      
      toast.success("EHR record updated successfully");
      router.push(`/dashboard/patients/${params.id}/ehr/${params.ehrId}`);
    } catch (error: any) {
      toast.error(error.message || "Failed to update EHR record");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="p-6 bg-background">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold">
          Edit EHR Record for {patient.firstName} {patient.lastName}
        </h1>
        <Button
          variant="outline"
          onClick={() => router.push(`/dashboard/patients/${params.id}/ehr/${params.ehrId}`)}
        >
          Cancel
        </Button>
      </div>
        ...
        <Card className="mb-6">
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? "Updating..." : "Update EHR Record"}
            </Button>
          </CardFooter>
        </Card>
      </form>
